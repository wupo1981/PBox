#!/usr/bin/env python3
"""
Script to add X symbol in the middle of crosspointgain icons
"""
from PIL import Image, ImageDraw, ImageFont
import os

icon_dir = r"c:\cc\Project\PB\Streamdeck\pbox\com.aurawave.pbox.sdPlugin\imgs\actions\crosspointgain"
...
def add_x_to_icon(file_path, is_2x=False):
    """Add X symbol in the middle of the icon"""
    try:
        ...
        # Draw X symbol in white with black outline for better visibility
        x_text = "X"
        ...
        for adj_x in [-2, -1, 0, 1, 2]:
            for adj_y in [-2, -1, 0, 1, 2]:
                if adj_x != 0 or adj_y != 0:
                    draw.text((x + adj_x, y + adj_y), x_text, font=font, fill="black")
        
        # Draw white text on top
        draw.text((x, y), x_text, font=font, fill="white")
...
print("Adding X symbol to crosspointgain icons...\n")
...
for base_file, x2_file in files_to_process:
    ...
    if os.path.exists(base_path):
        add_x_to_icon(base_path, is_2x=False)
    else:
        print(f"✗ File not found: {base_file}")
    
    if os.path.exists(x2_path):
        add_x_to_icon(x2_path, is_2x=True)
    else:
